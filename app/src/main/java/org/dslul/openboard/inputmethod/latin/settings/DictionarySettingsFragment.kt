...
class DictionarySettingsFragment : SubScreenFragment() {
    ...
    private fun reloadDictionaries() {
        ...
        //  in each: alphabetic by locale string
        // for now, don't sort at all
            ...
            builder.setNeutralButton(if (currentDictExistsInternal) R.string.reset_dictionary else R.string.delete_dict) { _, _ ->
                AlertDialog.Builder(DialogUtils.getPlatformDialogThemeContext(activity))
                    ...
                    .setPositiveButton(R.string.delete_dict) { _,_ ->
                        currentDictLocale?.getUserDictFilename()?.let {
                            val f = File(it)
                            f.delete()
                            if (f.parentFile?.list()?.isEmpty() == true)
                                f.parentFile?.delete()
                        }
                        reloadDictionaries()
                    }
    ...
    private fun onDictionaryFileSelected(resultCode: Int, resultData: Intent?) {
        ...
        if (shouldAskMessageId != 0)
            showConfirmReplaceDialog(locale, shouldAskMessageId)
        else
            moveCachedFileToDictionaries(locale)
    }

    private fun showConfirmReplaceDialog(locale: Locale, messageId: Int) {
        AlertDialog.Builder(DialogUtils.getPlatformDialogThemeContext(activity))
    ...
    private fun moveCachedFileToDictionaries(locale: Locale) {
        val dictFile = File(locale.getUserDictFilename())
        ...
        // success, now remove internal dictionary file (as it should be effectively replaced)
        //  maybe later we could enable adding expansion dictionaries...
    ...
    private fun Locale.getUserDictFilename() =
    ...
    companion object {
        ...
        private const val DICTIONARY_URL =
            "https://github.com/openboard-team/openboard/" // TODO: update once it exists
